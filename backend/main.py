...
# Import modular components from core
from core.config import llm
from core.database import vector_db, parent_store
from core.rag import do_hybrid_search, custom_stuff_documents, prompt_template
from core.format_checker import check_docx_format, fix_docx_format, check_pdf_format
from viva_agents import EXAMINERS_CONFIG

# 2. KHỞI TẠO FASTAPI
app = FastAPI(title="Thesis Chatbot API")

# 3. CẤU HÌNH CORS (Cho phép React cổng 3000 gọi sang Python cổng 8000/8001)
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
Câu hỏi: {user_query}"""
        ...
        # Import local to handle the response extraction safely
        from core.rag import extract_text
...
Câu hỏi: {user_query}"""
            ...
            docs = do_hybrid_search(user_query)
...
# Format checking and fixing endpoints
